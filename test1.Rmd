...
moves_str <- "e4 d5 d3 e5 Qf3 Qf6 h3 Qxf3 g4 Bc5 b3 Qxf2+ Kd1 a6 h4 Bxg4+ Nf3 Bxf3+ Be2 Qxe2#"
stockfish_path <- "/Program Files (x86)/Tarrasch/Engines/stockfish_11_x64"
results <- py$analyze_each_move(moves_str, depth=20, stockfish_path=stockfish_path)
...
names(results_df) <- c("Move", "Score")
```

```{r}
# Add a MoveNumber column to the data frame
results_df$MoveNumber <- seq_len(nrow(results_df))

# Plot using MoveNumber as the x-axis and log scale for the y-axis
ggplot(data = results_df, aes(x = MoveNumber, y = as.numeric(Score))) + 
    geom_line() +
  ylim(-10,10)+
    theme_minimal()
